...
fn image_and_pull_policy() -> (String, &'static str) {
    ...
    // Only a binary that came out of the release workflow can name an image
    // that exists. A local build inherits Cargo.toml's version, which the
    // release process never bumps, so it would name a stale tag with total
    // confidence — the very skew this is here to prevent.
    let from_release = env!("MINITCP_FROM_RELEASE") == "1";
    if !from_release || release.is_empty() || release == "0.0.0" {
...
mod tests {
    ...
    #[test]
    fn a_local_build_never_pins_to_a_version_it_only_guessed() {
        // Cargo.toml's version is not bumped on release, so outside the
        // release workflow it names a tag that is real but old — the worst
        // case, because it fails silently rather than loudly.
        if env!("MINITCP_FROM_RELEASE") != "1" {
            let (image, pull) = image_and_pull_policy();
            assert_eq!(image, format!("{IMAGE_REPO}:latest"), "{image}");
            assert_eq!(pull, "always");
        }
    }
