...
pub fn reply_for(request: &[u8]) -> Option<[u8; 28]> {
    ...
    let hardware_type =
        u16::from_be_bytes(request[0..2].try_into().ok()?); // [0..2] HTYPE: what a "hardware address" is
    let protocol_type =
        u16::from_be_bytes(request[2..4].try_into().ok()?); // [2..4] PTYPE: what a "protocol address" is
    let operation =
        u16::from_be_bytes(request[6..8].try_into().ok()?); // [6..8] OPER: question vs answer
    ...
    let requester_mac: [u8; 6] =
        request[8..14].try_into().ok()?; // [8..14] SHA: who asked (sender's hardware address, also Ethernet destination in main.rs)
    let requester_ip: [u8; 4] =
        request[14..18].try_into().ok()?; // [14..18] SPA: sender's protocol address
